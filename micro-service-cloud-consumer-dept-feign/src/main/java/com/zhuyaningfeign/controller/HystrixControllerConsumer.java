...
@Slf4j
@RestController
//@DefaultProperties(defaultFallback = "deptGlobalFallbackMethod") //全局的服务降级方法
public class HystrixControllerConsumer {

    private final DeptHystrixService deptHystrixService;

    @Autowired
    public HystrixControllerConsumer(DeptHystrixService deptHystrixService) {
        this.deptHystrixService = deptHystrixService;
    }
...
//    @HystrixCommand
    ...
    public String deptTimeoutHandler(@PathVariable("id") Integer id) {
        ...
        return "服务端系统繁忙，请稍后再试！（客户端降级 deptInfo_Timeout 专属的回退方法触发，id = " + id + "）";
    }
